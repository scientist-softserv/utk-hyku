...
RSpec.describe Hyrax::IiifHelper, type: :helper do
  ...
  describe '#iiif_viewer_display_partial' do
    ...
    context "with #iiif_viewer override" do
      let(:iiif_viewer) { :clover_iiif }

      before do
        allow(presenter).to receive(:iiif_viewer).and_return(iiif_viewer)
      end

      it { is_expected.to eq 'hyrax/base/iiif_viewers/clover_iiif' }
    end
  end
  ...
  describe '#clover_iiif_base_url' do
    subject { helper.clover_iiif_base_url }

    it 'defaults to clover-iiif base path' do
      expect(subject).to eq "http://test.host/clover-iiif"
    end
  end

  describe '#clover_iiif_config_url' do
    subject { helper.clover_iiif_config_url }

    it 'defaults to clover-iiif config path' do
      expect(subject).to eq "http://test.host/clover-iiif/clover-iiif-config.json"
    end
  end
end
