...
RSpec.describe 'Admin can select dc repository theme', type: :feature, js: true, clean: true do
  ...
  context "as a repository admin" do
    it 'sets the trace repository theme when the theme form is saved' do
      ...
      click_link('Themes')
      select('DC', from: 'Home Page Theme')
      find('body').click
      click_on('Save')
      site = Site.last
      account.sites << site
      allow_any_instance_of(ApplicationController).to receive(:current_account).and_return(account)
      expect(site.home_theme).to eq('dc_repository')
    end
  end
end
